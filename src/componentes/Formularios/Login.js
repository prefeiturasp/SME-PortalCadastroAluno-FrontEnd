...
export const Login = () => {
  ...
  useEffect(() => {
    ...
    const listaCodEolBloqueadoStorage = localStorage.getItem(
  ...
  useEffect(() => {
    localStorage.setItem(
      "listaCodEolBloqueado",
      JSON.stringify(listaCodEolBloqueado)
    );
  }, [listaCodEolBloqueado]);

  const handleBtnAAbrirFormularioDisable = () => {
    return (
      btnDisable === true || inputCodigoEol === "" || inputDtNascAluno === ""
    );
  };
  ...
  const buscaDadosAluno = (inputCodigoEol, inputDtNascAluno) => {
    buscaDadosAlunoResponsavel(inputCodigoEol, inputDtNascAluno)
      .then(retorno_api => {
        if (
          retorno_api.detail ===
            "Data de nascimento invalida para o código eol informado" ||
          retorno_api.detail === "API EOL com erro. Status: 404" ||
          retorno_api.detail === "API EOL com erro. Status: 500"
        ) {
          ...
          retorno_api.detail ===
          "Este estudante não faz parte do público do programa de uniforme escolar"
        ) {
          mensagem.setAbrirModal(true);
          mensagem.setTituloModal("Dados inválidos, tente novamente");
  ...
  const verificaCodEolBloqueado = codEol => {
    let listaCodEolBloqueadoStorage = localStorage.getItem(
      "listaCodEolBloqueado"
    );
    // Converte este json para objeto
          ...
          <div className="row">
            <div className="col-lg-4 mt-4">
              ...
              {errors.codigoEol && errors.codigoEol.type === "required" && (
                <span className="span_erro text-white mt-1">
                  Código EOL é obrigatório
                </span>
              )}
              {errors.codigoEol && errors.codigoEol.type === "maxLength" && (
                <span className="span_erro text-white mt-1">
                  Permitido até 10 dígitos
                </span>
              )}
            </div>
            <div className="col-lg-4 mt-4">
              ...
              <input
                ...
                max="9999-12-31"
              />
              {errors.dtNascAluno && errors.dtNascAluno.type === "required" && (
                <span className="span_erro text-white mt-1">
                  Data de nascimento do estudante é obrigatório
                </span>
              )}
              {errors.dtNascAluno &&
                errors.dtNascAluno.type === "maxLength" && (
                  <span className="span_erro text-white mt-1">
